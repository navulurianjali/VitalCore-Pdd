...
export function computeDigitalTwin(metrics: BaseHealthMetrics, chronologicalAge: number = 30): HealthDigitalTwin {
  ...
  return {
    ...
    burnoutRisk: (sleepHrs > 0 || hydroMl > 0 || stepsVal > 0) ? burnout : 0,
    fatigueBuildup: (sleepHrs > 0 || hydroMl > 0 || stepsVal > 0) ? fatigue : 0,
    sleepDeteriorationRisk: sleepHrs > 0 ? sleepRisk : 0,
    recoveryDeclineRisk: metrics.recoveryPercentage > 0 ? recoveryRisk : 0,
    trackingDaysCount: metrics.trackingDaysCount ?? 0,
    hasTelemetry: Boolean((sleepHrs > 0 || hydroMl > 0 || (metrics.caloriesConsumed && metrics.caloriesConsumed > 0) || stepsVal > 0)),
    hasEnergyTelemetry: Boolean((sleepHrs > 0 || hydroMl > 0 || (metrics.caloriesConsumed && metrics.caloriesConsumed > 0) || stepsVal > 0)),
