...
export class ScreenshotUtil {
  ...
  /**
   * Capture a screenshot via Appium driver, or generate a placeholder in simulate mode.
   */
  public static async capture(
    ...
    try {
      if (driver) {
        // Real Appium screenshot via WebdriverIO
        const screenshot = await driver.takeScreenshot();
        const imgBuffer = Buffer.from(screenshot, 'base64');
        fs.writeFileSync(filepath, imgBuffer);
        Logger.info(`Screenshot saved: ${filename}`);
      } else {
        // Placeholder (simulate mode): write a minimal 1x1 PNG
