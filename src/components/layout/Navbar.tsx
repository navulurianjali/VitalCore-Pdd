...
import { Activity, Sun, Moon, LogOut, Menu, X, ArrowRight } from "lucide-react";
...
export const Navbar: React.FC = () => {
  ...
  // Navbar is only active on public routes (/, /features, /auth/*)
  // Authenticated app routes use DashboardLayout's own header - Navbar returns null there.
  const isDashboard = Boolean(user) && !["/", "/features"].includes(pathname) && !pathname.startsWith("/auth");

  // PUBLIC LANDING PAGE ("/") must ALWAYS show Log In / Sign Up — never authenticated controls.
  // This is a public marketing page regardless of whether a Supabase session exists.
  const isPublicLandingPage = pathname === "/";
  const showAuthButtons = isPublicLandingPage || !user || loading;
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-[var(--border)] bg-[var(--background)]/90 backdrop-blur-md">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-[72px] items-center justify-between">
          ...
          <div className="hidden md:flex items-center justify-end md:w-[250px] gap-4">
            <button
              onClick={toggleTheme}
              className="flex items-center justify-center p-2 rounded-xl border border-[var(--border)] bg-[var(--card-bg)] text-[var(--foreground)] hover:text-primary transition-colors cursor-pointer"
            ...
            {showAuthButtons ? (
              <div className="flex items-center gap-3">
                <Link
                  href="/auth/login"
                  className="text-[14px] font-semibold text-[var(--foreground)] hover:text-primary transition-colors px-4 py-2 rounded-xl border border-foreground/10 hover:border-primary/30"
                >
                  Log In
                </Link>
                <Link href="/auth/signup">
                  <Button variant="primary" size="md" className="px-5 rounded-xl font-semibold shadow-sm">
                    Sign Up
                  </Button>
                </Link>
              </div>
              ...
              <div className="flex items-center gap-3">
                <Link href="/dashboard">
                  <Button variant="primary" size="md" className="px-5 rounded-xl font-semibold shadow-sm flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="text-[var(--muted)] hover:text-rose-500 transition-colors flex items-center gap-1 text-xs font-semibold cursor-pointer px-2.5 py-2 rounded-xl hover:bg-rose-500/10"
                  title="Log Out"
                >
                ...
                <div className="border-t border-[var(--border)] my-3 pt-3 flex flex-col gap-2">
                  {showAuthButtons ? (
                    ...
                    <>
                      ...
                      <Button variant="glass" onClick={handleSignOut} className="w-full text-rose-500 border-rose-500/20">
